...
bool SortedSet::search(TComp elem)  {
    TElem current = this->root;
    while (current != NULL_TELEM) {
        if (current == elem)
            return true;
...
int SortedSet::getIndex(TElem elem) {
...
TElem SortedSet::getLeft(TElem elem) {
...
TElem SortedSet::getRight(TElem elem) {
    int index = getIndex(elem);
...
TElem SortedSet::getParent(TElem elem) {
    int index = getIndex(elem);
    if (index != -1 && index != 0) {
...
void SortedSet::setLeft(TElem elem, TElem left) {
    int index = getIndex(elem);
    if (index != -1 && (2 * index + 1) < length) {
...
void SortedSet::setRight(TElem elem, TElem right) {
    int index = getIndex(elem);
    if (index != -1 && (2 * index + 2) < length) {
